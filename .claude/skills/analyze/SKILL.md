---
name: analyze
description: 个股深度分析。当用户说"分析XX"、"看看XX怎么样"、"XX值得买吗"、"研究一下XX"时使用此skill。支持A股、港股、美股分析。
user-invocable: true
---
...
| 优先级 | 来源 | 用途 | 支持市场 |
|--------|------|------|----------|
| **1** | `fetch_full_analysis.py` | 宏观+行业+技术全套数据 | A股、港股、美股 |
| **2** | 用户配置文件 | 持仓、关注方向、投资风格 | 全市场 |
| **3** | WebSearch | **仅用于**财报、公告、研报 | 全市场 |

### 市场支持说明

| 市场 | 宏观分析 | 行业分析 | 技术分析 | 资金流向 | 财务数据 |
|------|----------|----------|----------|----------|----------|
| A股 | ✅ | ✅ | ✅ | ✅ | ✅ |
| 港股 | ✅ | ✅ | ✅ | ❌ | ❌ |
| 美股 | ❌ | ❌ | ✅ | ❌ | ❌ |
...
**美股说明**：
- 美股仅支持技术面分析（均线、MACD、RSI、ATR等）
- 不支持宏观环境和行业分析（数据源限制）
- 不支持资金流向和财务数据（需要付费API）
...
**美股：**
```bash
cd "股市信息" && python3 scripts/fetch_full_analysis.py AAPL --market us
```

**注意**：脚本会自动识别市场类型（港股以0开头5位数字，美股为大写字母）
...
| 龙虎榜 | ❌ | ✅ | ❌ | ❌ |
| 成分股 | ✅ | ❌ | ❌ | ❌ |
| 分析重点 | 宏观+行业+趋势 | 全面分析 | 宏观+行业+技术 | 纯技术分析 |
