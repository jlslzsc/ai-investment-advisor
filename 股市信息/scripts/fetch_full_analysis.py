...
# 导入新的数据获取模块
try:
    from tushare_fetcher import fetch_a_stock_history as ts_fetch_history
...
try:
    from fmp_fetcher import fetch_historical_prices as fmp_fetch_history
    FMP_AVAILABLE = True
    log("✓ FMP美股数据模块已加载")
except ImportError:
    FMP_AVAILABLE = False
...
# 导入美股宏观与行业数据模块
try:
    from us_market_fetcher import (
        fetch_us_indices,
        fetch_us_market_trend,
        fetch_us_market_sentiment,
        fetch_us_sector_performance,
        fetch_us_sector_analysis
    )
    US_MARKET_FETCHER_AVAILABLE = True
    log("✓ 美股宏观与行业数据模块已加载")
except ImportError:
    US_MARKET_FETCHER_AVAILABLE = False
    log("⚠ 美股宏观与行业数据模块未找到")
...
def fetch_stock_data(code, market="a"):
    ...
    try:
        ...
        if market == "us":
            if FMP_AVAILABLE:
                try:
                    hist_data = fmp_fetch_history(code, from_date='2023-01-01')
            ...
            if code.startswith('5') or code.startswith('1'):
                ...
                if TUSHARE_AVAILABLE:
                    try:
                        ...
                        df = ts_fetch_history(code, start_date, end_date)
                        if df is not None and not df.empty:
                            return df
                    except Exception as e:
                        log(f"Tushare获取失败，回退到AKShare: {e}")
...
def full_analysis(code, market="a"):
    ...
    # 美股使用专门的宏观和行业分析
    if market == "us":
        if US_MARKET_FETCHER_AVAILABLE:
            log("【美股】获取宏观与行业数据...")
            # 宏观环境
            ...
            result["macro"] = {
                ...
                "trend": trend,
                "sentiment": sentiment,
                "score": trend.get('score', 10)
            }

            # 行业分析
            sectors = fetch_us_sector_performance()
            sector_analysis = fetch_us_sector_analysis()

            result["sector"] = {
                "performance": sectors,
                "analysis": sector_analysis,
                "score": sector_analysis.get('score', 10)
            }
        else:
            log("【美股】宏观与行业数据模块不可用，跳过")
            result["macro"] = {"note": "美股宏观数据模块不可用"}
            result["sector"] = {"note": "美股行业数据模块不可用"}
    else:
        # 1. 宏观环境
    ...
    if market == "us":
        ...
        if US_MARKET_FETCHER_AVAILABLE:
            ...
            # 如果宏观行业模块不可用，仅基于技术面评分
            result["score"] = {
                "total": result["technical"].get("trend", {}).get("score", 0) * 10 + 50,
                "level": "技术面分析",
                "suggestion": "仅供参考",
                "breakdown": {
                    "macro": {"score": 0, "max": 20, "detail": ["模块不可用"]},
                    "sector": {"score": 0, "max": 20, "detail": ["模块不可用"]},
