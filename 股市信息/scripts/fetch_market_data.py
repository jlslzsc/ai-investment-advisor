...
# 尝试导入新的数据获取模块
try:
    from fmp_fetcher import fetch_quote as fmp_fetch_quote, batch_fetch_quotes as fmp_batch_quotes
    FMP_AVAILABLE = True
    log("✓ FMP美股数据模块已加载")
except ImportError:
    FMP_AVAILABLE = False
    log("⚠ FMP模块未找到，将使用旧的美股模块")

try:
    from tushare_fetcher import (
        fetch_a_stock_quote as ts_fetch_a_quote,
        batch_fetch_a_stocks as ts_batch_a_stocks,
        fetch_hk_stock_quote as ts_fetch_hk_quote,
        batch_fetch_hk_stocks as ts_batch_hk_stocks
    )
    TUSHARE_AVAILABLE = True
    log("✓ Tushare数据模块已加载")
except ImportError:
    TUSHARE_AVAILABLE = False
    log("⚠ Tushare模块未找到，将使用AKShare")
...
def fetch_a_stock_data(holdings_stock, watchlist_items=None):
    ...
    if TUSHARE_AVAILABLE:
        try:
            # 批量获取持仓A股
            if holdings_stock:
                codes = list(holdings_stock.keys())
                ts_quotes = ts_batch_a_stocks(codes)

                for code, info in holdings_stock.items():
                    if code in ts_quotes:
                        quote = ts_quotes[code]
                        price = quote['price']
                        holdings_result.append({
                            "code": code,
                            "name": info["name"],
                            ...
                            "pb": quote.get('pb', 0),
                            "market_cap": quote.get('total_mv', 0),
                            "float_market_cap": quote.get('circ_mv', 0),
                            "cost": info["cost"],
                            "qty": info.get("qty", 0),
                            "pnl": calc_pnl(info["cost"], price),
                            "days": calc_days(info["buy_date"])
                ...
                if code:
                    ...
                    if quote:
                        ...
                        base.update({
                            ...
                            "pb": quote.get('pb'),
                            "market_cap": quote.get('total_mv'),
                            "float_market_cap": quote.get('circ_mv'),
                        })
                        watchlist_result.append(base)

            log(f"✓ 使用Tushare获取A股数据: 持仓{len(holdings_result)}只, 关注池{len(watchlist_result)}只")
            return holdings_result, watchlist_result

        except Exception as e:
            log(f"Tushare获取A股失败，回退到AKShare: {e}")
...
def fetch_hk_stock_data(holdings_hk, watchlist_items=None):
    ...
    try:
        ...
        log(f"✓ 使用AKShare获取港股数据: 持仓{len(holdings_result)}只, 关注池{len(watchlist_result)}只")
    except Exception as e:
        log(f"获取港股失败: {e}")
    return holdings_result, watchlist_result
    ...
    if FMP_AVAILABLE:
        try:
            ...
            for item in watchlist_items:
                ...
                quote = fmp_fetch_quote(code)
                if quote:
                    base = build_watchlist_base(item)
                    base["code"] = code
                    if not base["name"]:
                        base["name"] = quote.get('name', code)
                    base.update({
                        "price": quote['price'],
                        "change": quote['change'],
                        "change_pct": quote['change_pct'],
            ...
            log(f"✓ 使用FMP获取美股数据: 持仓{len(holdings_result)}只, 关注池{len(watchlist_result)}只")
            return holdings_result, watchlist_result

        except Exception as e:
            log(f"FMP获取美股失败，回退到旧模块: {e}")
